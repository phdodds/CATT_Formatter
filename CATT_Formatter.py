...
for filename in os.listdir(folder): #for every file in the directory
    if filename.endswith('.geo'): #if the file has a .geo extension
       newname = filename.replace('.geo', '.txt') #replace the .geo with a .txt
       output = os.rename(filename, newname) #rewrite the file in the folder
       
       with open(newname,'r') as file: #open the txt file
           filedata = file.read()
...
for filename in os.listdir(folder): #for every file in the directory
    if filename.endswith('.txt'): #if the file has a .txt extension
       newname = filename.replace('.txt', '.geo') #replace the .txt with a .geo
       output = os.rename(filename, newname) #rewrite the file in the folder
